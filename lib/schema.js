...
exports.getSchema = async function (templateConf = {}) {
    let schema = parseConfToSchema(templateConf);
    let metaSchema = store.get('metaSchema');

    if (!metaSchema) {
        metaSchema = await this.getMetaSchema();
    }

    return _.merge({}, metaSchema, schema);
};
...
exports.getJsonSchema = async function (templateConf = {}) {
    let jsonSchema = parseConfToJsonSchema(templateConf);
    let metaJsonSchema = store.get('metaJsonSchema');

    if (!metaJsonSchema) {
        metaJsonSchema = await this.getMetaJsonSchema();
    }

    return _.merge({}, metaJsonSchema, jsonSchema);
};
