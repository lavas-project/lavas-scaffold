 ...
 */
function renderTemplate(params, tmpStoreDir, isStream) {
    let template = store.get('templateConf');
    let dirPath = params.dirPath || process.cwd();
    let etplCompile = new etpl.Engine(template.etpl || conf.ETPL);

    // 把指定的文件和文件夹都删掉
    deleteFilter(tmpStoreDir, template.exportsIgnores);
    ...
    return new Promise((resolve, reject) => {
        glob(
            ...
            {
                cwd: tmpStoreDir,
                ignore: (template.renderIgnores || []).concat(...conf.DEFAULT_RENDER_IGNORES)
            },
            (err, files) => {
                files.forEach(file => {
 ...
 * 获取模版信息
 *
 * @param {string} frameworkValue 指定的 framework
 * @param {string} templateValue 指定的模版
 * @return {Object} framework 和 template 信息
 */
async function getTemplateInfo(frameworkValue, templateValue) {
    let meta = await getMeta();
    let framework;
    let template;

    for (let item of meta.frameworks) {
        if (item.value === (frameworkValue || meta.defaults.framework)) {
            framework = item;
        }
    }
...
/**
 * 下载某一个模版 (mata 信息中必须包含 framework 和 template)
 *
 * @param {Object} params  导出模版所需字段
 * @return {any}              导出的结果
 */
exports.download = async function (params = {}) {
    let {framework, template} = await getTemplateInfo(params.framework, params.template);
    let gitRepo = template.git;
    let storeDir = path.resolve(conf.LOCAL_TEMPLATES_DIR, framework.value, template.value);
    let ajv = new Ajv({allErrors: true});
    let metaJsonSchema = await schema.getMetaJsonSchema();
    ...
    try {
        ...
        store.set('dir', storeDir);

        let templateConfContent = fs.readFileSync(path.resolve(storeDir, 'meta.json'), 'utf-8');
        let templateConf = JSON.parse(templateConfContent);
        store.set('templateConf', templateConf);

        return templateConf;
    }
    catch (e) {
        throw new Error('git 下载模版出错，请检查当前网络。');
    }
};
 ...
 * @param {boolean} isStream 导出的形式是否的 zip 包
 * @return {*} 导出的结果
 */
exports.render = async function (params) {
    let templateConf = store.get('templateConf');
    let tmpStoreDir = path.resolve(conf.LOCAL_TEMPLATES_DIR, `${Date.now()}`);
    let storeDir = store.get('dir');
    let ajv = new Ajv({allErrors: true});
    let jsonSchema = await schema.getJsonSchema(templateConf);
